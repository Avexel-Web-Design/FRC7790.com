...
export async function getMessages(c: Context): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      channelReadStatuses = (readRes.results ?? []).map(r => {
        const row = r as { user_id: number; last_read_timestamp: string; username: string };
        return { user_id: Number(row.user_id), last_read_timestamp: row.last_read_timestamp, username: row.username };
      });
    } catch (e) {
  ...
  if (results.length > 0) {
      try {
        ...
        const otherMaxRead = (otherReadRow as { max_ts: string | null } | null)?.max_ts ?? null;

        const otherReadTime = otherMaxRead ? new Date(otherMaxRead).getTime() : null;
        for (const r of results as Record<string, unknown>[]) {
          if (r.sender_id === userId) {
            const msgTime = new Date(r.timestamp as string).getTime();
            r.read_by_any = otherReadTime !== null && otherReadTime >= msgTime ? 1 : 0;
          }
        }
      } catch (e) {
    ...
    // Mark channel as read for the requesting user
    if (results.length > 0) {
      try {
        ...
        // Non-critical: don't fail message retrieval if read status update fails
      }
    }
...
export async function sendMessage(c: Context): Promise<Response> {
  const { channelId } = c.req.param();
  const senderId = getAuthenticatedUserId(c);
  ...
  try {
    ...
    if (isPrivateChan) {
      ...
      const admRow = await c.env.DB.prepare('SELECT is_admin FROM users WHERE id = ?').bind(senderId).first();
      const isAdmin = admRow && (admRow as { is_admin: number }).is_admin === 1;
      if (!isAdmin) {
        const membRow = await c.env.DB.prepare('SELECT 1 FROM channel_members WHERE channel_id = ? AND user_id = ?').bind(channelId, senderId).first();
        if (!membRow) {
          return new Response('Forbidden', { status: 403 });
        }
      }
    ...
    const result = await c.env.DB.prepare(
      ...
      .bind(channelId, senderId, content, timestamp)
      .run();
    ...
    if (result.success) {
      ...
      try {
        await c.env.DB.prepare(`
          ...
          ON CONFLICT(user_id, channel_id) DO UPDATE SET
            last_read_timestamp = excluded.last_read_timestamp
        `).bind(senderId, channelId, timestamp).run();
      } catch {
        // Non-critical: don't fail message send if read status update fails
      }

      // Fire push notifications (best-effort)
      try {
        const senderRow = await c.env.DB.prepare('SELECT username FROM users WHERE id = ?').bind(senderId).first();
        const senderName = (senderRow as { username: string } | null)?.username || 'New message';
        await sendChannelMessagePush(c, channelId, senderId, senderName, content);
      } catch {
        // Push is best-effort
      }
...
export async function deleteMessage(c: Context): Promise<Response> {
  ...
  try {
    ...
    const isAdmin = adminRow && (adminRow as { is_admin: number }).is_admin === 1;
    const isAuthorized = isAdmin || (message as { sender_id: number }).sender_id === userId;
...
export async function getDMMessages(c: Context): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      const otherLastRead = (otherReadRow as { last_read_timestamp: string | null } | null)?.last_read_timestamp ?? null;
      const otherReadTime = otherLastRead ? new Date(otherLastRead).getTime() : null;
      for (const r of results as Record<string, unknown>[]) {
        if (r.sender_id === userId) {
          const msgTime = new Date(r.timestamp as string).getTime();
          r.read_by_any = otherReadTime !== null && otherReadTime >= msgTime ? 1 : 0;
...
export async function sendDMMessage(c: Context): Promise<Response> {
  ...
  try {
    ...
    const result = await c.env.DB.prepare(
      ...
      .bind(dmId, senderId, content, timestamp)
      .run();
    ...
    if (result.success) {
      ...
      try {
        await c.env.DB.prepare(`
          ...
          ON CONFLICT(user_id, channel_id) DO UPDATE SET
            last_read_timestamp = excluded.last_read_timestamp
        `).bind(senderId, dmId, timestamp).run();
      } catch {
        // Non-critical
      }
      ...
      try {
        const senderRow = await c.env.DB.prepare('SELECT username FROM users WHERE id = ?').bind(senderId).first();
        const senderName = (senderRow as { username: string } | null)?.username || 'New message';
        await sendChannelMessagePush(c, dmId, senderId, senderName, content);
      } catch {
        // Push is best-effort
      }
