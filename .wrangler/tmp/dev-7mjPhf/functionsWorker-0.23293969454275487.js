...
// .wrangler/tmp/bundle-CLQGtf/checked-fetch.js
...
// .wrangler/tmp/pages-Un1dKH/functionsWorker-0.23293969454275487.mjs
...
users.put("/:userId", async (c) => {
  ...
  try {
    ...
    if (body.username !== void 0 && body.username.trim() !== "") {
      const existingUser = await c.env.DB.prepare("SELECT id FROM users WHERE username = ? AND id != ?").bind(body.username.trim(), userId).first();
      if (existingUser) {
    ...
    if (body.password !== void 0 && body.password.trim() !== "") {
      const hashedPassword = await hashPassword2(body.password.trim());
  ...
  try {
    ...
    if (channelId.startsWith("dm_")) {
      ...
      if (members.length) {
        ...
        const all = (userRows.results || []).map((r) => Number(r.id));
...
profile.get("/", async (c) => {
  try {
    const user = c.get("user");
    const dbUser = await c.env.DB.prepare("SELECT id, username, is_admin, created_at, avatar_color, user_type FROM users WHERE id = ?").bind(user.id).first();
    if (!dbUser) {
...
profile.put("/", async (c) => {
  try {
    ...
    if (body.password) {
      ...
      const hashedPassword = await hashPassword3(body.password);
      const { success } = await c.env.DB.prepare(
...
preferences.delete("/teams/:team_number", async (c) => {
  ...
  await c.env.DB.prepare("DELETE FROM user_team_preferences WHERE user_id = ? AND team_number = ?").bind(user.id, team_number).run();
  return c.json({ success: true });
});
...
function humanCompLevel(match2) {
  switch (match2?.comp_level) {
    case "qm":
...
function buildStats(match2) {
  if (!match2) {
    return { prompt: "No match data provided.", fallback: "Summary unavailable.", factors: [] };
  }
  const blueTeams = match2?.alliances?.blue?.team_keys?.map((t) => t.replace("frc", "")).join(", ");
  const redTeams = match2?.alliances?.red?.team_keys?.map((t) => t.replace("frc", "")).join(", ");
  const blueScore = match2?.alliances?.blue?.score;
  const redScore = match2?.alliances?.red?.score;
  const winner = match2?.winning_alliance;
  const key = match2?.key;
  const round = humanCompLevel(match2);
  const base = `${round} ${key}: Blue [${blueTeams}] ${blueScore} - Red [${redTeams}] ${redScore}. Winner: ${winner || "TBD"}.`;
  const breakdown = match2?.score_breakdown;
  const factors = [];
...
ai.post("/generate", async (c) => {
  try {
    ...
    try {
      ...
      return c.json({ error: "Invalid JSON in request body", details: parseErr.message }, 400);
    }
    const { match_key, match: match2 } = body;
    if (!match_key && !match2) {
      return c.json({ error: "match_key or match object required" }, 400);
    }
    let matchData = match2;
...
// .wrangler/tmp/bundle-CLQGtf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CLQGtf/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.23293969454275487.js.map
