...
// ../.wrangler/tmp/bundle-vx54Q9/checked-fetch.js
...
tasks.put("/:id", async (c) => {
  try {
    ...
    ).bind(title, description || null, is_completed ? 1 : 0, due_date || null, id).run();
    if (success) {
...
profile.get("/", async (c) => {
  try {
    const user = c.get("user");
    const dbUser = await c.env.DB.prepare("SELECT id, username, is_admin, created_at FROM users WHERE id = ?").bind(user.id).first();
    if (!dbUser) {
...
// ../.wrangler/tmp/pages-KTFpY5/functionsRoutes-0.20249746023488102.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-vx54Q9/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-vx54Q9/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7765114253492975.mjs.map
